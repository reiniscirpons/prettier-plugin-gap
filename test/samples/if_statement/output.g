...
if Length(e) = 0 then
  return OneOfPcp(Pcp(H));
fi;
if opt[3] then
  AddRowVector(Q[i], Q[j], -q);
fi;
if not x in Source(i) then
  return fail;
fi;
if m = fail then
  TryNextMethod();
...
if r = 0 then
  return Image(GhomE, R!.elts[d]) * Image(HhomE, S!.elts[Order(H)]);
fi;
if Length(free) > 0 then
  Add(ref, B);
fi;
if Size(S) <> Size(CharacterTable("Fi23")) then
  Error("wrong size");
fi;
if (P mod 9) <> 1 then
  return false;
...
if y > 0 then
  Append(ans, GBNP.TransLetter(y));
fi;
if not IsParentLiePRing(L) then
  return fail;
fi;
...
if IsMutable(u) or IsMutable(u[1]) then
  MyError(190);
fi;
if IsMutable(u) or IsMutable(u[1]) then
  MyError(98);
...
if Length(x) > 10 then
  Print("ArrayValueFunctions needs to be implemented for longer lists\n");
  return fail;
fi;
if IsProbablyPrimeInt(m) then
  Add(StandardFactorsList[1], m);
else
  Add(StandardFactorsList[2], m);
fi;
if not Size(IndeterminatesOfPolynomialRing(pring)) = 1 + pg!.dimension then
  Error(
    "The dimension of the projective space should be 1 less than the number of indeterminates of the polynomial ring"
  );
fi;
if Length(MM) = 0 then
...
if not (
      IsGraph(ptgraph) and IsInt(s) and IsInt(t) and IsInt(nspaces) and
      IsInt(printlevel)
    ) or (IsBound(arg[6]) and not IsList(arg[6])) then
  Error(
    "usage: PartialLinearSpaces(",
    "<Graph>, <Int>, <Int>, [<Int>, [<Int>, [<List>]]])"
  );
fi;
if 1 in grid[i]{[1 .. j]} then
  if 1 in grid[i]{[j .. len]} then
    if 1 in List([1 .. i], x -> grid[x][j]) then
      if 1 in List([i .. len], x -> grid[x][j]) then
        return true;
      fi;
    fi;
  fi;

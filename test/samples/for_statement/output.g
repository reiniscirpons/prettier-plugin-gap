...
od;
for i in setup.els[j] do
  ConvertToMatrixRep(i);
od;
for i in [1 .. n] do
  AVLIndexAdd(t, i + 17, true, 2 * i);
od;
for i in [1 .. (Size(F) - 1) / q[Size(q)]] do
  Add(inp, elm[i]);
od;
for x in [1 .. 2] do
  return;
...
for i in [1 .. Length(rules)] do
  Info(
    InfoCoh, 1, "First rule ", i, ", ", Length(BasisVectors(eqs)), " equations"
  );
  l1 := rules[i][1];
  len1 := Length(l1);
  ...
  if Length(olen) > 3 and olen[Length(olen)] = olen[Length(olen) - 2] and
      olen[Length(olen)] <> olen[Length(olen) - 3] then
    k := List(BasisVectors(eqs), ShallowCopy);
    ;
    TriangulizeMat(k);
    eqs := MutableBasis(field, List(k, x -> ImmutableVector(field, x)), zeroq);
  fi;
od;
for a in [1, P - 1] do
  for c in [0 .. P - 1] do
...
for c in candidates do
  ...
  if Length(rclass) > 1 then
    pres := Concatenation(
        pres,
        List([2 .. Length(rclass)], i -> Set([rclass[1][1], rclass[i][1]]))
      );
  fi;
od;

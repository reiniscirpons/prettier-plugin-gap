repeat d := NextIterator(iter); until Size(d) > 1;

repeat until fertig or procrels();

repeat r := NextIterator(iter); until Size(r) > 1;

repeat d := NextIterator(iter); until Size(d) > 1;

repeat x:= PseudoRandom( der ); until Order( x ) = 40;
...
repeat s:= Random( g3 ); until Order( s ) = 15;
...
repeat x:= Random( g2 ); until Order( x ) = 14;

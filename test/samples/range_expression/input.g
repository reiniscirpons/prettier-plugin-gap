...
[ cg.commutator + 1 ..
              cg.commutator + cg.centralizer ];

[ t.dynamic.topleft[2]
                       + ( t.dynamic.topleft[2] mod 2 ) .. selected[2] - 1 ];

[minMiddleLength, 
                    minMiddleLength + granularity + 2 .. maxMiddleLength];
...
[ Position( spinsymname, '(' ) + 1
                            .. Length( spinsymname ) - 1 ];

[ N!.subspace.dimensionN + 1 ..
...
[1..Minimum(DegreeOfUnivariateLaurentPolynomial(pol),
                          Random([2,2,2,2,3,3,3,4,5,6]))];

[ 2^(k-1) - 2^((k+1)/2)/2
                                        .. 2^(k-1) - 2^( QuoInt(k,2)-1 ) ];

[1..Length(Defs{Filtered([1 .. Length(Defs)], x -> weights[x] < Maximum(weights))})];

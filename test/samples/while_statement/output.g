while h <= Length(d) and d[h] = 0 do
  h := h + 1;
od;
while cpos > 0 and (str[cpos] in " \t") do
  cpos := cpos - 1;
od;
while i > 0 and IsZero(v[pivs[i]]) do
  i := i - 1;
od;
while v[j] >= d[j] do
  v[j] := v[j] - d[j];
od;
while gaussCode[1][1][beg] > 0 do
  beg := beg + 1;
od;
while gaussCode[1][1][i + 1] > 0 do
  i := i + 1;
  Add(arc1, gaussCode[1][1][i]);
od;
while n mod (q * p) = 0 do
  q := q * p;
  l := l + 1;
od;
while j <= length and rel[j] = num1 do
  j := j + 1;
od;
while not IsOne(g ^ (p ^ e)) do
  g := g ^ p;
od;
while not IsDoneIterator(o) do
  Add(wts1, NextIterator(o));
od;
while Size(tempw) < Size(zlist[Size(zlist)]) do
  Add(tempw, 0);
od;
while not (IsIdle(wf)) do
  DoQueues(wf, true);
  Print(".\c");
od;
while v[j] < 0 do
  v[j] := v[j] + d[j];
od;
while i <= Length(M) and M[i][c] = zero do
  i := i + 1;
od;
while h <= Length(d) and d[h] = 0 do
  h := h + 1;
...
od;
while j < Length(dom) and IndexNC(dom[i - 1], dom[j + 1]) <= 2000 do
  j := j + 1;
od;
while iso = fail do
  i := i + 1;
  ...
  if Size(Group(gens)) = size then
    ...
    if p <> fail then
      iso := GroupHomomorphismByImagesNC(
          G, H, code.pcgs, CodeGenerators(gens2[p], spcgs2).pcgs
        );
    fi;
  ...
  if iso = fail then
    ...
    if Size(Group(gens)) = size then
      ...
      if p <> fail then
        iso := GroupHomomorphismByImagesNC(
            G, H, CodeGenerators(gens1[p], spcgs1).pcgs, code.pcgs
          );
      fi;
...
while i <= Length(n) - 1 do
  ...
  repeat
    ...
    if ForAny(
          p{ran},
          k -> ForAny(p{ran}, x -> DepthOfPcElement(p, Comm(x, k)) < n[j])
        ) then
      ea := false;
    fi;
...
od;
while q <> fail do
  if q = pos or cls[q].representative / cl.representative in K then
...
while i <= Size(word) do
  if word[i] <> '(' then
    ...
    if rbracket = Size(word) or (not word[rbracket + 1] = '^') then
      ...
      if p = 0 then
        ErrorNoReturn(
          "expected ^ to be followed", " by a positive integer but found 0"
        );
      fi;
      if product = "" then
        product := RemoveBrackets(word{[lbracket + 1 .. rbracket - 1]}) ^ p;
      else
        product := product *
          RemoveBrackets(word{[lbracket + 1 .. rbracket - 1]}) ^ p;
      fi;
      i := i - 1;
    fi;
  fi;

...
FindBasePointCandidates(Group(lgens),opt,ii,
	       false:Subrecurse := recsub,Facrecurse := recfac);

Print( "#E  ", p, "-modular table of '", Identifier( ordtblMGA ),
                  "' is missing\n" );

IsGeneratorsOfSemigroup(Concatenation(GeneratorsOfSemigroup(S),
                                                   coll));
...
DeclareOperation("LeadingTermOfPolynomial",
  [IsPolynomialFunction,IsMonomialOrdering,IsObject,IsObject,IsObject,IsObject,IsObject]);
...
AsCapCategoryMorphism(
  ...
  CAP_JIT_INCOMPLETE_LOGIC(
    Opposite(CAP_JIT_INCOMPLETE_LOGIC(deduped_3_1 ))
  ),
  UnionOfColumns(
    deduped_4_1,
    deduped_1_1,

rec(torsionCoefficients:=[ ]);

rec( label := [ [ 1, 2, 3 ], [ 0, 1 ] ], index := 25 );

rec(attr:=[],content:=["A","Address","Alt","Arg","B","Br","Button","C","Cite","Code","Display","E","Email","Emph","Enum","Example","F","File","Homepage","Ignore","Index","K","Keyword","Label","List","Listing","Log","M","Math","P","PCDATA","Package","Par","Q","Quoted","Ref","Table","URL","Verb"],name:="Subtitle",reqattr:=[],type:="mixed");

rec( pcp := pcp,conjugator := conjugator,gens_mod := gens_mod );

rec();

rec( label := [ 1, 1 ], index := 14 );

rec();

rec();

rec(success := true, result := exec.output);

rec(schreier := true, storenumbers := true);

rec();

rec( type:= [ 3, 2 ], f:= liealg_hom( L, K, [x,y,x*y], Basis( K ) ) );

rec(acting := true);

rec(acting := true);

rec( type:= [ 6, 31, b/c ], f:= liealg_hom( K, N, Basis( K ), imgs ) );

rec();

rec( cf := U, ti := [IdentityMat(d, F)] );
...
rec( rts0:= pi[k], rts1:= [ ], start:= 0,
                      sp:= VectorSpace( Rationals, pi[k], zero ) );

rec(D := DClass(R, MultiplicativeZero(R)),
                               number := true);
...
rec( text\/html := texthtml,
                             text\/plain := textplain,
                             metadata := rec( text\/html := "",
                                              text\/plain := "" ) );

rec(chevSys := [pv,nv,hs],
